...
<div class="flex h-screen overflow-hidden bg-[#F8FAFC] font-plus antialiased text-slate-900">
  ...
  >
    <div class="h-20 flex items-center px-7 shrink-0 overflow-hidden">
      <div class="w-10 h-10 rounded-2xl bg-indigo-600 flex items-center justify-center font-black text-white shadow-xl shadow-indigo-100 shrink-0 italic text-xl">K</div>
      {#if isSidebarOpen}
        <div class="ml-4 flex flex-col leading-tight whitespace-nowrap" transition:fade>
          <span class="text-base font-black tracking-tighter text-slate-800 uppercase">Khwarizmi</span>
          <span class="text-[10px] font-bold text-indigo-500 uppercase tracking-[0.3em]">Console v4</span>
        </div>
      {/if}
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-10 custom-scrollbar">
      <div>
        {#if isSidebarOpen}
          <p class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4" in:fade>Menu Utama</p>
        {/if}
        <nav class="space-y-2">
      ...
      <div>
        {#if isSidebarOpen}
          <p class="px-4 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4" in:fade>Laporan</p>
        {/if}
        <nav class="space-y-2">
    ...
    <div class="p-6">
      <div class="rounded-3xl bg-slate-50 p-2">
  ...
  <main class="flex-1 flex flex-col min-w-0 bg-[#F8FAFC]">
    <header class="h-20 flex items-center px-8 lg:px-12 sticky top-0 bg-[#F8FAFC]/80 backdrop-blur-xl z-30 justify-between">
      <div class="flex items-center gap-6">
        <button
          onclick={toggleSidebar}
          class="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-500 hover:text-indigo-600 hover:border-indigo-200 hover:shadow-lg hover:shadow-indigo-50 transition-all active:scale-90"
        >
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-2 mt-2">
            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Workspace</span>
            <span class="text-[10px] text-slate-300">/</span>
            <span class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{pageTitle.split(" ")[0]}</span>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4 lg:gap-8">
        <div class="hidden md:flex flex-col text-right">
          <p class="text-[11px] font-black text-slate-900 leading-none uppercase tracking-tighter">Luthfi Hakim</p>
          <p class="text-[10px] font-bold text-emerald-500 uppercase mt-1 tracking-widest flex items-center justify-end gap-1">
            <span class="w-1 h-1 rounded-full bg-emerald-500 animate-ping"></span> Online
          </p>
        </div>
        <div class="w-12 h-12 rounded-[1.25rem] bg-slate-900 flex items-center justify-center font-black text-white shadow-xl shadow-slate-200 border-2 border-white">LH</div>
      </div>
    </header>
    ...
    <div class="flex-1 overflow-y-auto px-8 lg:px-12 pb-12">
      <div class="max-w-[1400px] mx-auto w-full">
        {#key page.url.pathname}
          <div in:fly={{ y: 20, duration: 400, delay: 100 }} out:fade={{ duration: 100 }} class="mt-4">
            <slot />
          </div>
        {/key}
      </div>
